...
<x-page-header title="Notifications" subtitle="Stay updated on your progress and schedule">
    <div class="d-flex gap-2">
        <form action="{{ route('notifications.markAsRead') }}" method="POST">
            @csrf
            <button type="submit" class="btn btn-light border text-secondary btn-sm rounded-3">
                Mark all as read
            </button>
        </form>
        <form action="{{ route('notifications.destroyAll') }}" method="POST" onsubmit="return confirm('Are you sure you want to delete all notifications?')">
            @csrf
            @method('DELETE')
            <button type="submit" class="btn btn-light border text-danger btn-sm rounded-3">
                Delete all
            </button>
        </form>
    </div>
    ...
    <div class="card border-0 shadow-sm rounded-4 overflow-hidden mt-4">
        <div class="card-body p-0">
            @forelse($notifications as $notification)
                ...
                @endphp
                <div class="notification-item p-4 d-flex align-items-start gap-3 {{ $notification->unread() ? 'unread' : '' }}">
                    ...
                    <div class="flex-grow-1">
                        ...
                        @if(isset($data['task_id']))
                            <a href="{{ route('tasks.show', $data['task_id']) }}" class="btn btn-sm btn-outline-primary rounded-pill px-3 py-1 fw-bold" style="font-size: 0.75rem;">
                                View Task
                            </a>
                        @endif
                    </div>
