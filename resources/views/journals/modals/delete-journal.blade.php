      ...
      <form action="{{ route('journals.destroy', $journal->id) }}" method="POST">
        ...
        @method('DELETE')
          <div class="modal-body text-center pt-4">
            ...
            <p class="text-muted mb-4">{{ __('Are you sure you want to delete ') }}{{ $journal->title }}?<br>{{ __('This action cannot be undone.') }}</p>
          </div>

          <div class="d-flex justify-content-center px-3 pb-3 gap-2">
            <button type="button" class="btn btn-light rounded-pill px-4 fw-semibold border text-muted shadow-sm" data-bs-dismiss="modal">{{ __('Cancel') }}</button>
