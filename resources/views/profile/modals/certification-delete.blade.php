    ...
    <div class="modal-content p-3 border-0 shadow-lg rounded-4">
      <form method="POST" action="{{ route('profile.certification.destroy', $cert->id) }}">
        ...
        <div class="modal-body text-center p-4">
          ...
          <h5 class="modal-title mb-4 fw-bold text-dark">{{ __('Delete Certification') }}</h5>
          <p class="text-muted mb-4">{{ __('Are you sure you want to delete the certification:') }} <strong>{{ $cert->title }}</strong>?</p>
