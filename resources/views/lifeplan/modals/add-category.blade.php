                        ...
                        <div class="d-flex flex-wrap gap-2 icon-selection-grid">
                            ...
                            @foreach($presetIcons as $id => $iconClass)
                                <label class="icon-radio-label">
                                    <input type="radio" name="icon_id" value="{{ $id }}" class="d-none icon-radio" required {{ $id == 1 ? 'checked' : '' }}>
                        ...
                        <div class="d-flex flex-wrap gap-2 color-selection-grid">
                            ...
                            @foreach($presetColors as $id => $hex)
                                <label class="color-radio-label">
                                    <input type="radio" name="color_id" value="{{ $id }}" class="d-none color-radio" required {{ $id == 1 ? 'checked' : '' }}>
...
<style>
    /* Styling for the icon radio buttons */
    .cursor-pointer {
        cursor: pointer;
    }
    .icon-box {
    ...
    /* Styling for the color radio buttons */
    .color-box {
        transition: all 0.2s ease;
    }
    .color-radio:checked + .color-box {
        border-color: #111827 !important; /* Dark border when selected */
        transform: scale(1.1);
    }
</style>
